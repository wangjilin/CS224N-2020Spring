...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_var = pad_sents_char(charindices, 0) # (batch_size, max_sentence_length, max_word_length)
        sents_var = torch.tensor(sents_var,dtype=torch.long,device=device).permute(1,0,2).contiguous()
