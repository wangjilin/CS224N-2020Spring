...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # CLASStorch.nn.Parameter, A kind of Tensor that is to be considered a module parameter.
    ...
    def embedding_lookup(self, w):
        ...
        # reference https://pytorch.org/docs/stable/nn.html#torch.nn.Embedding
        # print(w)
		# each input tensor is part of a sentence with n_features words
        # w (batch_size x n_features) x embeddings(num_words x embedding_size) 
        # => x (batch_size x embedding_size)
        
        temp = [torch.index_select(self.embeddings, 0, w[i]) for i in range(w.size()[0])]
        x = torch.cat(temp,0)
		#x.size() = (batch_size,n_features*self.embed_size)
    ...
    def forward(self, w):
        ...
        embd = self.embedding_lookup(w) # (batch_size,n_features*self.embed_size)		
        hidden_l = torch.matmul(embd,self.embed_to_hidden_weight)+self.embed_to_hidden_bias
        hidden_relu = nn.functional.relu(hidden_l)
        dropped_out = self.dropout(hidden_relu)
        logits = torch.matmul(dropped_out,self.hidden_to_logits_weight) + self.hidden_to_logits_bias
        ### END YOUR CODE
        return logits
